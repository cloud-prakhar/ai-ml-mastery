...
| Examples run single-threaded | `scripts/check_examples.py` sets `OMP_NUM_THREADS`, `OPENBLAS_NUM_THREADS`, `MKL_NUM_THREADS` and `NUMEXPR_NUM_THREADS` to 1; outputs are authored the same way | Thread count changes the summation order in parallel numeric code, so t-SNE, lbfgs and k-NN results differed in the last digit between a 16-core laptop and a 4-core CI runner. This, not the CPU kernel, kept CI red while every local and Docker run passed. Never print a timing comparison either — it flips on different hardware |
| Examples run as on a learner's machine | `scripts/check_examples.py` removes `CI`, `BUILD_NUMBER` and `GITHUB_ACTIONS` from each example's environment | pytest 8 prints a longer failure summary when it detects CI, which failed an example only on GitHub. When CI fails, read the `::error` annotations through the public checks API before guessing |
| Deep-learning dependency | PyTorch pinned in a separate `requirements-dl.txt` (`torch==2.14.0`), installed from the CPU wheel index; CI's `examples` job installs it, and both `pip-audit` steps check it with `--no-deps` | PyTorch is the largest dependency in the repository; learners on modules 00–07 never download it. The default Linux wheel pulls several gigabytes of CUDA libraries a CPU runner cannot use. The pin `==2.14.0` also matches the local build `2.14.0+cpu` |
| torchvision for vision modules | `torchvision==0.29.0` in `requirements-dl.txt`, the release that requires exactly `torch==2.14.0`. Architectures are built with `weights=None` — on the meta device when only shapes are needed — and published accuracy is read from weight metadata; examples never download weights or datasets | Real architecture definitions and reference box operations are worth a small pinned dependency; downloads would make CI depend on third-party hosts and caches |
| PyTorch examples in float64 | Training examples call `torch.set_default_dtype(torch.float64)`; chaotic runs print a band, not digits | Module 08's float32 examples gave different last digits under the generic and MKL-compatible CPU kernels after a few hundred training steps. In float64 they were identical under every kernel tested. A teaching choice, stated in the module overview, not a production recommendation |
...
- `06-feature-engineering` — fully authored: all 7 topics from the feature pipeline to leakage hunting and
  feature stores, a 68-question quiz with explained answers, and 4 assignments. Builds on module 03's encoding,
  leakage and feature-store topics by linking, not repeating; every leak it demonstrates is collected in topic 7
  as an automatic test.
- `07-model-evaluation` — fully authored: all 8 topics from cross-validation to ranking metrics, a 70-question quiz
  with explained answers, and 4 assignments. Every metric section includes "when this metric misleads".
  Bayesian optimisation is built from scikit-learn's Gaussian process; Optuna appears only as unexecuted
  reference code, following the unpinned-library decision above.
- `08-deep-learning` — fully authored: all 8 topics from the perceptron to diffusion models, graph networks and
  mixture-of-experts, a 64-question quiz with explained answers, and 4 assignments. The first module to execute
  PyTorch; attention and transformers are deliberately left to `11-transformers`.
- `09-computer-vision` — fully authored: all 6 topics from images as tensors to detectors, vision transformers, SAM and
  CLIP, a 70-question quiz with explained answers, and 4 assignments. Uses only scikit-learn's bundled images and
  synthetic data, and torchvision architectures built on the meta device without weights, so nothing is downloaded.
  Face recognition is taught with digits standing in for identities, so no personal data is used.
...
### 🚧 In progress
- Nothing currently mid-flight. Phase 2 (modules 00–04), Phase 3's modules (`05`–`07`), `08-deep-learning` and
  `09-computer-vision` are complete. Next in phase order: `10-natural-language-processing`. Phase 3's beginner projects
  B1–B10 remain open; B1, B2, B4 and B5 depend only on completed modules, and B7 is now unblocked by module 09.
- **Known gap:** CLAUDE.md asks for an image-generation prompt in `40-visual-learning/image-prompts/` for every major
  concept; module 09 has them, modules 00–08 have none yet.
