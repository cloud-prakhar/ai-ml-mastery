...
- `08-deep-learning/` authored as real content, and the first module to execute PyTorch. Every example verified by
  ...
  every Mermaid diagram rendered and viewed.
  - Topic 1: Neurons, Perceptrons and Layers — the perceptron learning AND and OR and never XOR; parameter counts;
    two stacked linear layers shown numerically equal to one
  - Topic 2: Loss Functions, Computational Graphs and Autograd — **an autograd engine in about 50 lines matching
    PyTorch's gradients**; squared error failing to train a neuron that cross-entropy fixes in 50 steps; a hand-written
    log-sigmoid returning infinity
  - Topic 3: The Training Loop, Batches and Initialisation — batch size compared by steps; learning-rate failure
    regions; **constant starting weights leaving 32 neurons acting as one**; Xavier against He through 10 layers
  - Topic 4: Vanishing Gradients, Normalisation, Dropout and Residuals — gradients measured per layer; batch norm
    depending on its batch; dropout's modest effect reported honestly; **at 30 layers only residual networks learned**
  - Topic 5: Activation Functions — gradients of seven activations; **dead ReLUs that neither training nor leaky ReLU
    revived**; softmax overflowing to `nan` without the max shift
  - Topic 6: MLPs, CNNs and Recurrent Networks — **a CNN keeping 96.5% on shifted digits where an equal-sized MLP fell
    to 50%**; gradient through time measured for RNN, LSTM and GRU, and the forget-gate bias that changes it
  - Topic 7: Autoencoders, VAEs and GANs — an autoencoder beating PCA; digits generated by a VAE and judged against a
    noise baseline; **GAN mode collapse across three seeds**
  - Topic 8: Transformers, Diffusion, GNNs and Other Architectures — **a diffusion model covering both modes that made
    the GAN collapse**; a graph network labelling 56 nodes from 4 labels; mixture-of-experts routing; Siamese and
    capsule networks
- `requirements-dl.txt` pins `torch==2.14.0` (CPU build), installed separately so modules 00–07 never download it.
  Verified by a clean install on Python 3.10. CI's `examples` job installs it; both `pip-audit` steps check it.
- `quizzes/08-deep-learning.md` (64 questions) with explained answers, and `assignments/08-deep-learning.md`
  (4 assignments).
- 33 glossary terms, from autoencoder to Xavier initialisation; 7 existing entries now link to Module 08 topics.
- `CLAUDE.md` gains the checks CI depends on: examples run with `CI=true`, alternative CPU kernels for numeric examples,
  float64 for PyTorch training, bands instead of unstable digits, Mermaid rendered and viewed, and the convention for
  heavy dependencies. `memory.md` records the PyTorch and float64 decisions.
- The root README's quick start mentions the deep-learning install.
